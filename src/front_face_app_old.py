...
path = r"C:\Users\saadi\Videos\Untitled_Project_V1.mp4"#video file path or 0 for webcam
face_width_mm = 125
#path = 1
#path = str(sys.argv[2])
#if path=='None':
...
def draw_lines(frame):
    if show_lines:
        if show_lines_Sn_sPog:
            #Orange
            cv2.line(frame, point_34, point_9, (0, 125, 255), 2, cv2.LINE_AA)
        if show_lines_sN_Sn:
            if avg_point:
                #Blue
                cv2.line(frame, avg_point, point_34, (255, 0, 0), 2, cv2.LINE_AA)
        #DarkGreen
        cv2.line(frame, point_22, point_23, (0, 100, 0), 2, cv2.LINE_AA)
        if show_lines_Ar_Right:
            #Cyan
            cv2.line(frame, point_3, point_4, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.line(frame, point_4, point_5, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.line(frame, point_5, point_6, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.line(frame, point_6, point_7, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.line(frame, point_7, point_8, (255, 255, 0), 2, cv2.LINE_AA)
            cv2.line(frame, point_8, point_9, (255, 255, 0), 2, cv2.LINE_AA)
        if show_lines_Ar_Left:
            #Red
            cv2.line(frame, point_9, point_10, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.line(frame, point_10, point_11, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.line(frame, point_11, point_12, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.line(frame, point_12, point_13, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.line(frame, point_13, point_14, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.line(frame, point_14, point_15, (0, 0, 255), 2, cv2.LINE_AA)
        if show_lines_iC_Left:
            #Purple
            cv2.line(frame, point_40, point_8, (255, 0, 255), 2, cv2.LINE_AA)
        if show_lines_iC_Right:
            #Light Green
            cv2.line(frame, point_43, point_10, (0, 255, 125), 2, cv2.LINE_AA)
        if show_lines_sZy:
            #Drawing Lines
...
while True:
    ...
    if len(faces) > 0:
        ...
        for i in range(68):
            ...
            # Display the landmark number around the circle
            #cv2.putText(frame, str(i+1), (x + 5, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 0), 1)
            # Display the landmark number around the circle with a different font
            font = cv2.FONT_HERSHEY_COMPLEX_SMALL  # Change to a different font
            font_scale = 0.5  # Adjust the font scale as needed
            font_color = (0, 0, 0)  # Black color
            font_thickness = 1  # Adjust the font thickness as needed

            # Use anti-aliased font (cv2.LINE_AA)
            #cv2.putText(frame, str(i+1), (x + 10, y - 10), font, font_scale, font_color, font_thickness, lineType=cv2.LINE_AA)
            ...
            if avg_point:
                cv2.circle(frame, (avg_point[0],avg_point[1]), 5, (0, 255, 0), -1)
                # Display the landmark number around the circle
                #cv2.putText(frame, str(i), (x + 10, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
